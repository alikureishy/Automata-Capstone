...
flags.DEFINE_string('output_path', None,
                    'Path to output record file, if split_train_test is enabled creates two file one for training and one for validation')
...
def main(unused_argv):
    ...
    if split_train_test:
        label_files_train, label_files_eval = train_test_split(label_files_train, test_size=split_train_test,
                                                               shuffle=True)
    ...
    if label_files_eval:
        ...
        print(
            'TF record file for validation created with {} samples: {}'.format(len(label_files_eval), output_path_eval))
